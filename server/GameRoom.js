...
export class GameRoom {
  constructor(roomCode, hostId, options) {
    ...
    this.players = [];
    this.gameState = { phase: 'waiting', currentPlayerIndex: 0, lastPlay: { type: 'none', cards: [], rank: 0, length: 0 }, lastPlayerId: null, pile: [], passCount: 0, finishOrder: [], round: 0, roles: {}, swapPending: {}, swapsCompleted: {} };
  }
  ...
  playCards(playerId, cardIndices) {
    ...
    if (player.hand.length === 0) {
      ...
      if (this.checkRoundEnd()) {
        this.endRound();
        return { success: true, playType, rounded: true };
      }
    }
  ...
  submitSwap(playerId, cardIndices) {
    ...
    this.checkAndProcessSwaps();
    return { success: true };
  }
  checkAndProcessSwaps() {
    const allDone = Object.keys(this.gameState.swapPending).every(id => this.gameState.swapsCompleted[id]);
    if (!allDone) return;
  ...
  getPublicState(requestingPlayerId) {
    ...
    return {
      ...
      players: this.players.map(p => ({
        id: p.id, name: p.name, isCPU: p.isCPU, handSize: p.hand.length, finished: p.finished, finishPosition: p.finishPosition, role: this.gameState.roles[p.id] || null, hand: p.id === requestingPlayerId ? p.hand.map(c => c.toJSON()) : null
      })),
