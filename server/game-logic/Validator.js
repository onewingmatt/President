...
export class Validator {
  static getPlayType(cards, options) {
    ...
    if (cards.length === 1) {
      return { type: 'single', cards: sorted, rank: RankSystem.rankValue(sorted[0], options), length: 1 };
    }
    const ranks = [...new Set(cards.map(c => c.rank))];
    if (ranks.length === 1) {
      return { type: 'set', cards: sorted, rank: RankSystem.rankValue(sorted[0], options), length: cards.length };
    }
