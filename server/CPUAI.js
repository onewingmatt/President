...
export class CPUAI {
  static decideTurn(hand, lastPlay, options) {
    ...
    if (!lastPlay || lastPlay.type === 'none') return { action: 'play', cardIndices: [0] };
    if (lastPlay.type === 'single') return this.tryBeatSingle(sorted, lastPlay.rank, options);
    if (lastPlay.type === 'set') return this.tryBeatSet(sorted, lastPlay.length, lastPlay.rank, options);
    return { action: 'pass', cardIndices: [] };
  }
  static tryBeatSingle(sorted, targetRank, options) {
    for (let i = 0; i < sorted.length; i++) {
      if (RankSystem.rankValue(sorted[i], options) > targetRank) return { action: 'play', cardIndices: [i] };
    }
    return { action: 'pass', cardIndices: [] };
  }
