...
function generateRoomCode() { return Array(6).fill(0).map(() => 'ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789'[Math.floor(Math.random() * 36)]).join(''); }
function cardToString(card) { const suits = { H: '♥', D: '♦', C: '♣', S: '♠' }; return card.rank + suits[card.suit]; }
function cardsToString(cards) { return cards.map(c => cardToString(c)).join(', '); }
function triggerCPUTurn(io, room, depth = 0) {
  ...
  setTimeout(() => {
    ...
    const prevIdx = (room.gameState.currentPlayerIndex - 1 + room.players.length) % room.players.length;
    const cpuPlayer = room.players[prevIdx];
    if ('playType' in result && result.playType) {
      const cardsPlayed = cardsToString(result.playType.cards);
      io.to(room.roomCode).emit('play-made', { playerId: cpuPlayer.id, playerName: cpuPlayer.name, playType: result.playType, cardsPlayed: cardsPlayed });
    } else {
      io.to(room.roomCode).emit('player-passed', { playerId: cpuPlayer.id, playerName: cpuPlayer.name });
    }
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('play-cards', (data) => {
      ...
      if (result.success) {
        ...
        const cardsPlayed = cardsToString(result.playType.cards);
        io.to(data.roomCode).emit('play-made', { playerId: socket.id, playerName: player.name, playType: result.playType, cardsPlayed: cardsPlayed });
        room.players.forEach(p => io.to(p.id).emit('game-state-update', room.getPublicState(p.id)));
    ...
    socket.on('pass-turn', (data) => {
      ...
      if (result.success) {
        const player = room.players.find(p => p.id === socket.id);
        io.to(data.roomCode).emit('player-passed', { playerId: socket.id, playerName: player.name });
        room.players.forEach(p => io.to(p.id).emit('game-state-update', room.getPublicState(p.id)));
        if (room.gameState.phase === 'playing') {
          const next = room.players[room.gameState.currentPlayerIndex];
          if (next.isCPU) setTimeout(() => triggerCPUTurn(io, room), 400);
